...
const GROUP_LABELS: Record<string, { label: string; icon: typeof Copy; color: string }> = {
  AI: { label: 'AI', icon: Wand2, color: '#a78bfa' },
  Clipboard: { label: 'Clipboard', icon: Copy, color: '#3b82f6' },
  Style: { label: 'Style', icon: Paintbrush, color: '#06b6d4' },
  Transform: { label: 'Transform', icon: Box, color: '#f59e0b' },
  Visibility: { label: 'Visibility', icon: Eye, color: '#22c55e' },
  Open: { label: 'Open Panel', icon: Layout, color: '#8b5cf6' },
  Danger: { label: 'Destructive', icon: Trash2, color: '#ef4444' },
};
        ...
        >
          ...
          <div style={{ padding: '8px 12px 10px', borderBottom: '1px solid rgba(255,255,255,0.05)', marginBottom: '2px' }}>
            <div className="flex items-center gap-2.5">
              ...
              >
                <TypeIcon className="h-3 w-3" style={{ color: '#0073E6' }} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-semibold truncate" style={{ color: '#ddd' }}>
                  {selectedElement.name || selectedElement.type}
                </div>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="text-[8px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded" style={{ color: '#0073E6', backgroundColor: 'rgba(0,115,230,0.08)' }}>
            ...
            return (
              <div key={group}>
                ...
                <div className="flex items-center gap-1.5 px-3 pt-1.5 pb-0.5">
                  <span style={{ fontSize: '8px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: groupMeta?.color || '#444', opacity: 0.6 }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: isHovered
                          ? item.danger ? 'rgba(239,68,68,0.1)' : item.highlight ? 'rgba(167,139,250,0.1)' : 'rgba(0,115,230,0.08)'
                          : 'transparent',
                        cursor: 'pointer',
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor: isHovered
                            ? item.danger ? 'rgba(239,68,68,0.12)' : item.highlight ? 'rgba(167,139,250,0.12)' : 'rgba(0,115,230,0.12)'
                            : 'rgba(255,255,255,0.03)',
                          border: `1px solid ${isHovered ? (item.danger ? 'rgba(239,68,68,0.15)' : item.highlight ? 'rgba(167,139,250,0.15)' : 'rgba(0,115,230,0.15)') : 'rgba(255,255,255,0.04)'}`,
                        }}
                      >
                        <Icon className="h-3 w-3" style={{ color: item.danger ? '#ef4444' : item.highlight ? '#a78bfa' : isHovered ? '#0073E6' : '#777' }} />
                      </div>
                      <div className="flex-1 min-w-0 text-left">
                        <span style={{
                          fontSize: '12px',
                          fontWeight: item.highlight ? 600 : 500,
                          color: item.danger ? '#ef4444' : item.highlight ? '#a78bfa' : isHovered ? '#fff' : '#bbb',
                          display: 'block',
                        }}>
                          {item.label}
                        </span>
          ...
          <div style={{ padding: '6px 12px', borderTop: '1px solid rgba(255,255,255,0.03)', marginTop: '2px' }}>
            <span style={{ fontSize: '8px', fontFamily: 'monospace', color: '#2a2a2a' }}>
