  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold tracking-tight flex items-center gap-2">
            <Globe className="h-4 w-4 text-muted-foreground" /> Multi-currency and FX
          </h2>
        ...
        <Button size="sm" className="h-8 rounded-lg text-xs gap-1.5" onClick={() => setShowAdd(true)}>
          <Plus className="h-3.5 w-3.5" /> Add rate
        </Button>
      </div>

      {/* Revaluation card */}
      <div className="p-4 rounded-[10px] bg-card border border-border/60">
        ...
        <div className="flex items-end gap-2">
          <div>
            ...
            <Input type="date" value={revalDate} onChange={e => setRevalDate(e.target.value)} className="h-9 w-[160px] rounded-lg mt-1" />
          </div>
          <Button size="sm" className="h-9 rounded-lg text-xs gap-1.5" disabled={revaluing} onClick={revalue}>
        ...
        <div className="p-4 rounded-[10px] bg-card border border-border/60">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 items-end">
            <div>
              ...
              <Input type="date" value={form.rate_date} onChange={e => setForm({ ...form, rate_date: e.target.value })} className="h-9 rounded-lg mt-1" />
            </div>
            <div>
              <Label className="text-[10px]">From</Label>
              <Input value={form.from_currency} onChange={e => setForm({ ...form, from_currency: e.target.value.toUpperCase() })} maxLength={3} className="h-9 rounded-lg mt-1 uppercase" />
            </div>
            <div>
              <Label className="text-[10px]">To</Label>
              <Input value={form.to_currency} onChange={e => setForm({ ...form, to_currency: e.target.value.toUpperCase() })} maxLength={3} className="h-9 rounded-lg mt-1 uppercase" />
            </div>
            <div>
              <Label className="text-[10px]">Rate (1 {form.from_currency || '–'} = ? {form.to_currency || '–'})</Label>
              <Input type="number" step="0.00000001" value={form.rate} onChange={e => setForm({ ...form, rate: e.target.value })} className="h-9 rounded-lg mt-1 tabular-nums" placeholder="0.00" />
            </div>
            <div className="flex gap-2">
              <Button size="sm" className="h-9 rounded-lg text-xs flex-1" onClick={addRate}>Save</Button>
              <Button size="sm" variant="ghost" className="h-9 rounded-lg text-xs" onClick={() => setShowAdd(false)}>Cancel</Button>
            </div>
          </div>
      ...
      <div className="rounded-[10px] bg-card border border-border/60 overflow-hidden">
        <div className="grid grid-cols-[100px_80px_80px_1fr_100px_40px] font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground px-4 py-2 border-b border-border/60 bg-muted/20">
        ...
        {loading ? (
          ...
          rates.map(r => (
            <div key={r.id} className="grid grid-cols-[100px_80px_80px_1fr_100px_40px] items-center px-4 py-2 text-xs border-b border-border/60 last:border-0 hover:bg-foreground/[0.025]">
              ...
              <span className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">{r.source || 'manual'}</span>
              <button onClick={() => deleteRate(r.id)} className="text-muted-foreground hover:text-risk transition">
