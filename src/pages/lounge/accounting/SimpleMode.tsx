...
import {
  ...
  Loader2, CheckCircle2, AlertTriangle, Percent, Landmark, Plus, Send, ShieldCheck, History,
} from 'lucide-react';
...
export default function SimpleMode({ orgId, orgName, accounts, currency, onGoTo }: Props) {
  ...
  const actions = [
    { key: 'sale' as const, title: 'Record a sale', desc: 'Money coming in from a customer', icon: TrendingUp },
    { key: 'expense' as const, title: 'Record an expense', desc: 'A bill or purchase to pay', icon: TrendingDown },
    { key: 'receive' as const, title: 'Got paid', desc: 'Mark a customer invoice as paid', icon: ArrowDownRight },
    { key: 'pay' as const, title: 'Paid a bill', desc: 'Record a payment you made', icon: ArrowUpRight },
  ];
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="mb-1 flex items-center gap-2">
          <span className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">Simple mode</span>
        </div>
        <h2 className="text-[19px] font-semibold tracking-[-0.015em]">Keep your books tidy</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard label="Sales this month" value={money(thisMonthSales, currency)} loading={loading} />
        <KpiCard label="Expenses this month" value={money(thisMonthExpenses, currency)} loading={loading} />
        <KpiCard label="Profit" value={money(profit, currency)} tone={profit >= 0 ? 'text-ok' : 'text-risk'} loading={loading} />
        <KpiCard label="VAT collected (MTD)" value={money(vatOwed, currency)} loading={loading} onClick={() => onGoTo?.('vat')} />
      </div>

      {/* Send to accountant — the big automation button */}
      <div
        className="relative overflow-hidden rounded-[10px] border border-border/60 bg-card p-5"
      >
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-border/60 bg-sunken">
            <Send className="h-5 w-5 text-muted-foreground" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="mb-1 flex items-center gap-2">
              <span className="font-mono text-[10px] font-medium uppercase tracking-[0.14em] text-muted-foreground">One-click filing</span>
            </div>
            <h3 className="text-[15px] font-semibold tracking-[-0.01em]">Send my accounts to be assessed</h3>
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3 text-ok" /> Reconciled ledger</span>
              <span className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-ok" /> Auto-checked</span>
              <span className="flex items-center gap-1"><CheckCircle2 className="h-3 w-3 text-ok" /> Signed-off PDF</span>
            </div>
          </div>
          <div className="flex flex-col gap-2 shrink-0">
            ...
            <Button
              ...
              variant="ghost"
              className="h-9 rounded-lg gap-2 text-xs text-muted-foreground hover:text-foreground"
            >
      ...
      {/* Actions */}
      <div>
        <h3 className="mb-2.5 text-[13.5px] font-[550] tracking-[-0.01em]">What happened today?</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {actions.map(a => (
            <button
              key={a.key}
              onClick={() => setFlow(a.key)}
              className="rounded-[10px] border border-border/60 bg-card p-4 text-left transition-colors duration-150 hover:border-border"
            >
              <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-lg border border-border/60 bg-sunken">
                <a.icon className="h-4 w-4 text-muted-foreground" />
              </div>
              <div className="text-[13px] font-[550]">{a.title}</div>
      ...
      {/* Overdue */}
      {overdue.length > 0 && (
        <div className="rounded-[10px] border border-attend/30 bg-attend/5 p-4">
...
function KpiCard({ label, value, tone, loading, onClick }: any) {
  const Comp: any = onClick ? 'button' : 'div';
  return (
    <Comp onClick={onClick} className={cn('flex items-center justify-between gap-3 rounded-[10px] border border-border/60 bg-card px-4 py-3 text-left', onClick && 'transition-colors duration-150 hover:border-border')}>
      <span className="font-mono text-[10px] font-medium uppercase tracking-[0.13em] text-muted-foreground">{label}</span>
      <span className={cn('font-mono text-[15px] font-medium tabular-nums', tone || 'text-foreground')}>{loading ? '…' : value}</span>
    </Comp>
  );
}
...
function QuickLink({ icon: Icon, label, onClick }: any) {
  return (
    <button onClick={onClick} className="flex items-center gap-2 rounded-lg border border-border/60 bg-card hover:bg-card px-3 py-2 text-xs">
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
          ...
          <div>
            ...
            {customers.length > 0 ? (
              <Select value={customerId} onValueChange={(v) => { setCustomerId(v); if (v) setNewCustomerName(''); }}>
                <SelectTrigger className="mt-1 h-10 rounded-lg"><SelectValue placeholder="Choose customer" /></SelectTrigger>
                <SelectContent>
                  {customers.map((c: any) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                </SelectContent>
              </Select>
            ) : null}
            <Input value={newCustomerName} onChange={e => { setNewCustomerName(e.target.value); if (e.target.value) setCustomerId(''); }} placeholder="…or type a new customer name" className="mt-2 h-10 rounded-lg" />
          </div>
          <div>
            <Label className="text-xs">What was it for?</Label>
            <Textarea value={desc} onChange={e => setDesc(e.target.value)} rows={2} placeholder="e.g. Website design work" className="mt-1 rounded-lg" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs">Amount (excl. VAT)</Label>
              <Input type="number" step="0.01" value={amount} onChange={e => setAmount(e.target.value)} className="mt-1 h-10 rounded-lg" />
            </div>
            <div>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={busy}>{busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save and post'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
          ...
          <div>
            ...
            {suppliers.length > 0 ? (
              <Select value={supplierId} onValueChange={(v) => { setSupplierId(v); if (v) setNewSupplierName(''); }}>
                <SelectTrigger className="mt-1 h-10 rounded-lg"><SelectValue placeholder="Choose supplier" /></SelectTrigger>
                <SelectContent>
                  {suppliers.map((s: any) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
                </SelectContent>
              </Select>
            ) : null}
            <Input value={newSupplierName} onChange={e => { setNewSupplierName(e.target.value); if (e.target.value) setSupplierId(''); }} placeholder="…or type a new supplier name" className="mt-2 h-10 rounded-lg" />
          </div>
          <div>
            <Label className="text-xs">What was it for?</Label>
            <Textarea value={desc} onChange={e => setDesc(e.target.value)} rows={2} placeholder="e.g. Office supplies" className="mt-1 rounded-lg" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs">Amount (excl. VAT)</Label>
      ...
      <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              {bankAccounts.length === 0 ? (
                <div className="mt-1 text-xs text-attend">Add a bank account in Banking first.</div>
              ) : (
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={busy || items.length === 0 || bankAccounts.length === 0}>
            {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save and post'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
